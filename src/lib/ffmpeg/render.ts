...
/**
 * Browser-based renderer using ffmpeg.wasm.
 *
 * Strategy for MVP (keeps complexity manageable):
 *   1. For each clip, generate a filtergraph: trim the source to in/out,
 *      apply reframe (zoompan/crop), set 9:16 output.
 *   2. Concatenate all clips with concat demuxer.
 *   3. Burn captions via subtitle filter from generated ASS file.
 *   4. Encode to MP4 H.264 + AAC.
 *
 * NOTE: This is a simplified pipeline. Full quality parity with after-effects-style
 *       caption animations requires WebCodecs + offscreen canvas — see
 *       lib/ffmpeg/render-advanced.ts (TODO).
 */
...
export interface RenderProgress {
  stage: 'loading' | 'fetching' | 'cutting' | 'rendering' | 'done' | 'error';
  message: string;
  progress: number;
}
...
export async function renderTimeline(
  ...
  ffmpeg.on('progress', ({ progress }) => {
    onProgress({ stage: 'rendering', message: 'Rendering…', progress });
  });
  ...
  // 2. Cut each clip into a separate segment
  ...
  for (let i = 0; i < timeline.clips.length; i++) {
    ...
    // Crop / reframe to 9:16. Use first keyframe center as static crop for MVP.
    ...
    const ox = Math.round(cx - cropW / 2);
    const oy = Math.round(cy - cropH / 2);

    await ffmpeg.exec([
      '-ss', c.in_seconds.toFixed(3),
      '-to', c.out_seconds.toFixed(3),
  ...
  // 4. Captions → ASS file
  const assContent = generateASS(timeline.captions, cfg.width, cfg.height);
  await ffmpeg.writeFile('captions.ass', assContent);

  // 5. Concat + burn captions
...
function generateASS(captions: Caption[], width: number, height: number): string {
  // Pink bold caption preset for ASS
...
`;
  const lines = captions
    .map((c) => {
      const text = c.words
        .map((w) => (w.highlight ? `{\\rHighlight}${w.word.toUpperCase()}{\\rViral}` : w.word.toUpperCase()))
        .join(' ');
